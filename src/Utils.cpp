...
[[noreturn]] void panic(std::string s) {
  ...
  std::cerr << c.foreground() << "[CRITICAL ERR] " << c.clear() << s
            << std::endl;
  std::exit(1);
}
...
void Utils::printLoading(unsigned current, unsigned total,
                         const std::string &message) {
  float percentage = std::round((float)current / total * 100);
  std::cout.flush();
  std::cout << "\r" << message << ": " << current / 1000 << "K / "
            << total / 1000 << "K (" << percentage << "%)     ";
}
...
double Utils::convertToRadians(double angle) { return angle * M_PI / 180; }

double Utils::haversineDistance(double lat1, double lon1, double lat2,
                                double lon2) {
...
void Utils::MSTdfsVisit(Vertex<Info> &v, std::vector<uint64_t> &res,
                        Graph<Info> &g) {
  v.setVisited(true);
